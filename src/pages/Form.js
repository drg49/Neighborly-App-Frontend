...
const Form = (props) => {
  ...
  const handleCreate = (e) => {
    ...
    if (file === null) {
      //If only text is added
      setPostBtn(<Spinner width={120} text="Uploading..." />);
      fetch(url + "/post/", {
        method: "post",
        headers: {
  ...
  return (
    <>
      <Link to={`/${city}/${state}/vault`}>
        <h2 id="vault-location">
          {city}, {state}
        </h2>
      </Link>
      <form id="create-form" onSubmit={handleCreate}>
        <div id="form-flex">
          <div>
            <label htmlFor="regular">Post Publicly</label>
            <br />
            <input type="radio" defaultChecked name="post" />
          </div>
          <div>
            <label htmlFor="regular">Post Anonymously</label>
            <br />
